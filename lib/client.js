  ...
  return {
    ...
    org: function(id) {
      return {
        ...
        // Create a new task
        create: function (org, done) {
          post(root + 'org', org, done);
        },

        // Get all org tasks
        getTasks: function (query, done) {
          get(root + 'org/' + id + '/tasks', query, done);
        },

        // Remove an org
        remove: function (done) {
          del(root + 'org/' + id, null, done);
        },

        // Check if org is over limit
        checkLimit: function (done) {
          get(root + 'org/' + id + '/limit', null, done);
        }
      };
...
function req (method, url, query, body, done) {
  request({
    ...
    if (res.statusCode > 299) {
      ...
      return done(new Error(message), body);
    }
    done(null, body);
  });
}
